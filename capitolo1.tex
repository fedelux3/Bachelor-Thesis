...
\\\\
In questo ramo della ricerca si inserisce il \textit{Data Mining}, ovvero l'insieme di tecniche e metodologie che hanno per oggetto l'estrazione di informazioni utili da grandi quantit\`a di dati attraverso metodi automatici o semi-automatici (\textit{machine learning})\cite{wiki:datamining}.\\
Le tecniche di data mining utilizzate nel lavoro svolto sono quelle delle \textit{regole di associazione}\cite{wiki:associaz}, ossia metodi per estrarre relazioni nascoste tra i dati.\\
...
Da una prima analisi questa associazione sembra completamente controintuitiva. Eppure questo \`e forse il pi\`u iconico degli esempi di applicazioni di data mining\cite{sole24:bigdata}. Infatti dall'analisi dei dati degli scontrini degli acquisti del fine settimana emergeva questa associazione, mettendo in luce il segmento delle coppie di giovani neo-genitori che invitavano gli amici a casa a cena nel weekend.\\
...
L'algoritmo preso in esame \`e lo \textit{Spatio-Temporal Breath-first Miner (STBFM)} definito da Piotr S. Maciag and Robert Bembenik\cite{icpram19}. Esso cerca di scoprire associazioni di tipologie di eventi connesse spazialmente e temporalmente.\\
...
\begin{itemize}
	...
	\item Nel \textbf{capitolo cinque} si analizzano i risultati ottenuti in termini di tempi di computazione e di significato degli indici
	
	\item Infine le \textbf{conclusioni e prospettive future}.	 
\end{itemize}
