...
\pagestyle{fancy}
\fancyhf{}
\rhead{\textit{3. Algoritmo}}
\cfoot{\thepage}

\section{Algoritmo di base}
Definiti i concetti teorici di base, ora verr\`a descritto l'algoritmo \textbf{Spatio-Temporal Breadth-First Miner} (STBFM) che permette di trovare tutte le sequenze pattern con \textit{participation index} maggiore di una certa soglia \textbf{threshold} $\theta$ data dall'utente.\\
...
A sostegno dell'algoritmo viene definita una struttura ad albero\cite{wiki:albero} dedicata per ridurre drasticamente il numero delle computazioni, in particolare per quanto riguarda la generazione di sequenze candidato nuove.

\subsection{Albero}
Innanzitutto definiamo cos'\`e una struttura ad albero orientato:\\
Un \textbf{albero orientato} \`e un grafo connesso, aciclico ed orientato\cite{cormen:libro}. Esso \`e una struttura dati che si compone di due sottostrutture fondamentali: 
\begin{itemize}
...
I nodi \textbf{foglia} sono i nodi che hanno solo archi entranti, quindi nessun arco uscente.
\begin{figure}[h]
	\centering
	\includegraphics[height=0.4 \linewidth]{pics/albero.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[height=0.5 \linewidth]{pics/sptree.png}
...
L'algoritmo proposto, non \`e l'unica alternativa allo studio di regole di associazioni. Un altro algoritmo di ricerca di associazioni molto valido \`e l'\textit{algoritmo apriori}.\\
\\
\textbf{L'algoritmo apriori} \`e utilizzato per la generazione di \textit{itemset} frequenti a partire da itemset con un solo elemento\cite{wiki:apriori}. Concretamente parte da una serie di itemset frequenti e vi aggiunge elementi verificando quanto questa frequenza venga mantenuta.\\
L'algoritmo si basa sulla considerazione che se un insieme di oggetti (itemset) \`e frequente,  allora anche tutti i suoi sottoinsiemi sono frequenti, viceversa se un itemset non \`e frequente, allora neanche gli insiemi che lo contengono sono frequenti, esso \`e definito \textbf{principio di anti-monotonicit\`a}.\\
\\
Viene applicato soprattutto per il \textit{problema del carrello della spesa}, ovvero lo studio delle correlazioni tra prodotti acquistati al supermercato. In questo tipo di situazioni la generazione del gruppo di candidati viene svolta aggiungendo un item per volta come quando si mettono i prodotti della spesa nel carrello. I gruppi di candidati sono successivamente valutati e l'algoritmo termina quando non ci sono ulteriori estensioni possibili, similmente a come avviene per lo \textit{STBFM}.\\
\\
...
\\
Ogni itemset cos\`i creato viene valutato all'interno del dataset, concetamente contando il numero delle istanze delle quali fa parte. Se il numero delle istanze supera un threshold $\epsilon$, l'itemset viene considerato per la generazione di itemset di lunghezza maggiore. Il processo \`e ripetuto finch\`e non \`e pi\`u possibile generare itemset significativi, rispettando il \textit{principio di anti-monotonicit\`a}.\\
Nel dettaglio vedi \textbf{Algorithm 9}.
\begin{algorithm}[h]
	\KwData{T - dataset delle transazioni, $\epsilon$ threshold di significativit\`a}
	
	\nl $L_1 := \{$ large $ 1-itemset\}$\\
	\nl $k:=2$\\
	\nl \While{$L_{k-1} \ne \emptyset$}{
		\nl $C_k :=$ Generate($L_{k-1}$)\\
		\nl \ForEach{$t \in T$}{
			\nl $C_t:=$ Subset($C_k$, $t$)\\
			\nl \ForEach{$c \in C_t$}{
				\nl $count[c] := count[c] + 1$\\
		...
		\nl $L_k := \{ c \in C_k | count[c] \ge \epsilon\}$\\
		\nl $k:=k+1$\\
	}
	\nl $return$ $\underset{k}{\bigcup} L_k$
	
	\caption{Algoritmo apriori}
\end{algorithm}
\paragraph{Considerazioni} Come possiamo notare questo algoritmo, con alcune dovute modifiche, \`e applicabile al nostro problema. Va notato, per\`o, che non vi \`e alcuna ottimizzazione per quanto riguarda la generazione di candidati, inoltre il threshold utilizzato ($\epsilon$) \`e fisso.\\
