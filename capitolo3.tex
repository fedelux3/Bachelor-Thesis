...
L'algoritmo proposto, non \`e l'unica alternativa allo studio di regole di associazioni. Un altro algoritmo di ricerca di associazioni molto valido \`e l'\textbf{algoritmo apriori}.
\paragraph{L'algoritmo apriori} \`e utilizzato per la generazione di \textit{itemset} frequenti a partire da itemset con un solo elemento. Concretamente parte da una serie di itemset frequenti e vi aggiunge elementi verificando quanto questa frequenza viene mantenuta.\\
L'algoritmo si basa sulla considerazione che se un inseime di oggetti (itemset) \`e frequente,  allora anche tutti i suoi sotto insiemi sono frequenti, viceversa se un itemset non \`e frequente, allora neanche gli insiemi che lo contengono sono frequenti.\\
\\
Viene applicato soprattutto per il \textit{problema del carrello della spesa}, ovvero lo studio delle correlazioni tra prodotti acquistati al supermercato. In questo tipo di situazioni la generazione del gruppo di candidati viene svolta aggiungendo un item per volta; i gruppi di candidati sono successivamente verificati e l'algoritmo termina quando non ci sono ulteriori estensioni possibili, compatibilmente come avviene per lo \textit{STBFM}.\\
\\
La generazione di itemset candidati consiste nella costruzione di un insieme ordinato di $k$ nodi, a partire da un itemset di grandezza $k-1$.\\
\textit{Esempio}

$A \rightarrow B \rightarrow C$

$A \rightarrow B \rightarrow D$\\
gli itemset generati saranno:

$A \rightarrow B \rightarrow C \rightarrow D$

$A \rightarrow B \rightarrow D \rightarrow C$

\paragraph{Considerazioni} Come possiamo notare questo algoritmo, con alcune dovute modifiche, \`e applicabile al nostro problema. Va notato, per\`o, che non vi \`e alcuna ottimizzazione per quanto riguarda la generazione di candidati, inoltre il threshold utilizzato ($\epsilon$) \`e fisso (vedi Algorithm 9).\\
