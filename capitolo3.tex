...
\noindent
\paragraph{Algorithm 1} L'algoritmo che sta alla base a tutto \`e l'Algorithm 1. Inizialmente genera le sequenze candidato di lunghezza 2, unendo ciascun tipo con tutti gli altri facendo tutte le combianzioni possibili. A tutte queste sequenze viene calcolato il \textit{set di istanze} e basandosi su questo si calcolano i \textit{participation index} (PI) attraverso la funzione VerifyCandidates.\\
...
La generazione avviene seguendo questa logica: \\
...
Come si pu\`o notare la nuova sequenza ha tutti i tipi della prima sequenza e in fondo l'ultimo tipo della seconda sequenza.\\
...
\paragraph{Algorithm 5} L'Algorithm 5 si occupa del calcolo del neighborhood. Esso viene applicato secondo la definizione data in precedenza, per\`o non prendendo i dati direttamente dal dataset. Si considera $I(\overrightarrow{s_i}[m-1])$ come set di partenza e si considera il vicinato con l'ultimo elemento della seconda sequenza $I(\overrightarrow{s_j}[m-1])$. 
\begin{algorithm}[h]
...
Come si pu\`o da subito notare il carico coputazionale pi\`u pesante lo si ha nell'Algorithm 3, la CandidateGen. In quell'algoritmo bisogna valutare tutte le possibili coppie di sequenze del livello precedente se sono compatibili tra loro per generare le nuove sequenze del livello successivo, fatto ci\`o calcolare il set di istanze. \\
Se consideriamo anche solo un dataset di 10.000 record con 20 tipi di eventi, significa che l'algoritmo itera sui 10.000 record migliaia di volte per ciascuna sequenze da generare. \\
